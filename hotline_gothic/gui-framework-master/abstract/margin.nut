	...
	constructor(arg = null)
	{
		if ("marginPx" in arg)
		{
			switch (typeof arg.marginPx)
			{
				...
				case "integer":
				{
					local value = arg.marginPx

					_marginPx = {top = value, right = value, bottom = value, left = value}
					break
				}
			}
		}
		else if ("margin" in arg)
		{
			switch (typeof arg.margin)
			{
				...
				case "integer":
				{
					local x = nax(arg.margin)
					local y = nay(arg.margin)

					_marginPx = {top = y, right = x, bottom = y, left = x}
					break
				}
			}
		}
